...
def startdkg():
    ...
    while 1:
        ...
        tempcontlist.append([parse.group(1), parse.group(2), parse.group(3),
                             parse.group(4), parse.group(5)])
    fp.close()
    [nid, c_ip, c_port, cert_file, l] = tempcontlist[int(nodeid)-1]
    ltr = ("./dkg/node %d certs/%d.pem certs/%d-key.pem files/contlist 0 0 0"
           % (int(c_port), int(nid), int(nid)))
    print nodeid, ltr
