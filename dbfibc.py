...
def main():
    ...
    # Phase 1 - Authenticating the identity. The following code will retreive
    # the username and password and send it to the module auth.py

    auth_type = raw_input("Please select the type of authentication "
    ...
    password = getpass.getpass("Please enter the password for your "
                               + auth_str + " '" + username + "' : ")
    auth_result = auth.auth(username, password, auth_type)
    auth_result = "S"
    if  auth_result is "S":
        print("Hello, " + username + ". Authentication has been successful. "
              "You can now use your username as your Identity\n")
    else:
    ...
    '''
    #Phase 2 - The DKG protocol can now start. It will run in a different
    thread. As soon as DKG completes, the share is written to a file
    print("Please wait while the system initializes")
    readlist()
    global nodeid
    dkg.dkg(nodeid)
    print "Waiting for DKG to generate key .....   ",
    while 1:
    ...
    # Phase 3 - Generate the keys using username and the share, the keys are
    # stored in the loaded ibc library
    ibc.start(username, nodeid)

    # Phase 4 - Applications. the generated keys can be used to encrypt as well
    # as decrypt messages
